...
This mode generates or updates four project governance files around a PRD (Product Requirements Document) and the current repository state. Together they give Claude Code a durable operating model for the repository. It behaves exactly like `prd-to-governance` 1.2.0 (its origin); the read-only audit part of that origin now lives in `audit.md`.

Shared vocabulary (uncertainty markers, priority levels, memory rules, the automation-contract fields): see `contract.md`. Do not restate it here.

For this mode, one project equals one folder. Treat the folder that contains the PRD as the default project root. Generate and maintain `SOUL.md`, `AGENTS.md`, `CLAUDE.md`, and `MEMORY.md` in that same folder. If the user explicitly says the PRD lives outside the real project folder, stop and ask which folder should be treated as the project root. After the user confirms that folder, use it consistently for all reads and writes.
...
- **Update/merge** - governance files already exist; audit them first (see `audit.md`, governance half), then merge PRD and codebase findings into the files the user selects
...
If the user only wants to review, compare, or check governance without changing it, that is the **audit** mode (`audit.md`), not govern.
...
- If one or more governance files already exist, run the governance audit first (`audit.md`), then propose **Update/merge**
...
6. **Automation**: Does the project use or plan CI/CD, auto-develop scripts, automated issue processing, or guardrail hooks?
   - If it uses (or plans to use) the pipeline generated by the automate mode, also ask two optional follow-ups; both are fail-safe and can be skipped (formats in `contract.md` sections 4 and 5):
...
- If the project uses (or will use) the automate mode, AGENTS.md is the producing side of two optional contracts that mode consumes (`contract.md` sections 4 and 5). Add them only when the behaviour is wanted; both are fail-safe when omitted (the pipeline runs unchanged):
...
9. **Update Rules** - how to maintain this file (the memory rules in `contract.md` section 3)
...
When governance files already exist, do not jump straight to rewriting them. Run the governance half of the audit as described in `audit.md` (read the four files, the PRD, and the repository; classify mismatches as Missing, Outdated, Contradicted by codebase, or Needs user decision; tag significant conflicts `[GOVERNANCE DRIFT]`; check the automation-contract coherence from `contract.md` section 6). Present the audit findings before proposing edits. Only after user approval, update the selected governance files with the merge strategy in Step 10.
...
If the user now wants the build loop automated, switch to the automate mode (`automate.md`); it requires all four files to exist.
