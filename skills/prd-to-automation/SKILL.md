...
One project equals one folder. The project root is the folder that contains the PRD (govern) or the four governance files (automate, audit). Read and write only inside it unless the user says otherwise. If the PRD lives outside the real project folder, sits in a documentation-only folder, or was pasted inline, stop and ask which folder is the project root before writing anything.
...
- **Required**: models are chosen explicitly by the user per pipeline step in automate, never silently inherited from governance.
...
Projects whose governance came from `prd-to-governance` 1.2.0 and whose script came from `governance-to-automation` 1.2.2 keep working unchanged. An audit reports no drift merely because the skill changed.
