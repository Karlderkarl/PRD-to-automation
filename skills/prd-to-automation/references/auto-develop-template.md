...
| `{{IMPL_RUNNER_CALL}}` / `{{REVIEW_A_RUNNER_CALL}}` / `{{REVIEW_B_RUNNER_CALL}}` | Step 3 confirmed model/CLI mapping | the runner body, prompt on stdin (`$1` is the prompt file): `claude -p --model "$MODEL" --permission-mode "$CLAUDE_PERMISSION_MODE" --output-format text < "$1"`; see *Reviewer runners* and *Headless permissions* under Generation rules |
...
```bash
#!/usr/bin/env bash
# Generated by prd-to-automation 1.1.0 (contract 1.1.0). Change it through /prd-to-automation automate (Sync).
set -euo pipefail
...
# label-less source) are warned and never arm the denylist base.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --review-b)        need_val "$@"; REVIEW_B_MODEL="$2";   shift 2 ;;   # omit for single-review
    --review-b-effort) need_val "$@"; REVIEW_B_EFFORT="$2";  shift 2 ;;   # omit for single-review
...
for cli in "$IMPL_RUNNER" "$REVIEW_A_RUNNER" "${REVIEW_B_RUNNER:-}"; do   # REVIEW_B_RUNNER is omitted for single-review
...
build_reexec_args() {
  ...
  # ${…:-}: the REVIEW_B_* defaults are omitted for single-review projects (set -u).
  [[ -n "${REVIEW_B_MODEL:-}" ]] && args+=(--review-b "$REVIEW_B_MODEL" --review-b-effort "${REVIEW_B_EFFORT:-high}")
...
run_review() {  # <label> <runner_fn> <issue> <title> <body> <logfile> <logdir> -> 0 pass, 1 findings, 2 runner failed, 3 tree modified
  local diff verdict re before rrc=0; diff="$(code_diff 2>/dev/null || true)"
  ...
  # Reviewers are read-only. A reviewer that edited the code (e.g. codex workspace-write) cannot be
  # undone cheaply, so it is a failed review (rc 3, no retry) and the issue fails via the normal path.
  [[ "$(code_hash)" == "$before" ]] || {
...
#     unprovable RED fails the issue; `preferred` always continues with advisory guidance only.
  ...
  #     by A/B. Reverts any round that is not cleanly approved; fails the issue only on an M7
