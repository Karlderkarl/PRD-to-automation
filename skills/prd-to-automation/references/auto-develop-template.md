...
# what `automate.md` Step 3 forbids without explicit opt-in. --unattended raises both
...
- **Memory rules are mandatory** (see `contract.md` section 3, *Memory rules*): the review diff must exclude `{{MEMORY_FILE}}`; the no-op `code_hash` comparison must exclude `{{MEMORY_FILE}}` and `$LOGDIR`; only `build_memory_update_prompt` writes the archive entry.
