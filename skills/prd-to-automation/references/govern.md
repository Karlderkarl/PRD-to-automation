# govern mode: PRD to governance files

This mode generates or updates four project governance files around a PRD (Product Requirements Document) and the current repository state. Together they give Claude Code a durable operating model for the repository. It behaves exactly like `prd-to-governance` 1.2.0 (its origin); the read-only audit part of that origin now lives in `references/audit.md`.

Shared vocabulary (uncertainty markers, priority levels, memory rules, the automation-contract fields): see `references/contract.md`. Do not restate it here.

For this mode, one project equals one folder. Treat the folder that contains the PRD as the default project root; when the PRD sits in a documentation folder inside a git repository, the default root is the repository's top-level folder (`SKILL.md` "Project root"). Generate and maintain `SOUL.md`, `AGENTS.md`, `CLAUDE.md`, and `MEMORY.md` in that same folder. If the user explicitly says the PRD lives outside the real project folder, stop and ask which folder should be treated as the project root. After the user confirms that folder, use it consistently for all reads and writes.
...
govern has two sub-modes:

- **Generate** - create or refresh governance files from the PRD, user decisions, and discovered repository reality
- **Update/merge** - governance files already exist; audit them first (see `references/audit.md`, governance half), then merge PRD and codebase findings into the files the user selects
...
Use **Update/merge** when:

- governance files already exist and the user asks to update, refresh, regenerate, merge, or align them

If the user only wants to review, compare, or check governance without changing it, that is the **audit** mode (`references/audit.md`), not govern.

## Workflow

Steps 5 to 8 draft the files; nothing is written before the approval in Step 11. Steps 9 and 10 apply only to Update/merge, where the audit of Step 9 runs before the interview (Step 0).

### Step 0: Select Sub-mode

Determine which sub-mode applies before reading deeply:

- If the repo has a PRD and none of `SOUL.md`, `AGENTS.md`, `CLAUDE.md`, `MEMORY.md` exist, default to **Generate**
- If one or more governance files already exist, run the governance audit first (`references/audit.md`), then propose **Update/merge**
- If the user explicitly says "create", "bootstrap", or "generate", prefer **Generate**
- If the user explicitly says "update" or "merge", prefer **Update/merge** (still audit first)
- If the user says "review", "compare", "check", or "audit" without asking for changes, switch to the audit mode

State the chosen sub-mode briefly to the user before proceeding.
...
- If the PRD lives in a documentation folder (for example `docs/`, `docs/prd/`, `planning/specs/`, or `.github/`) inside a git repository, the default project root is the repository's top-level folder (`git rev-parse --show-toplevel`); state it in one line. Outside a git repository, or when the PRD is nested deeper than a documentation folder, pause and confirm the project root before writing files
...
- **If any exist in Generate:** stop and tell the user which files were found; ask explicitly per file:
...
- **If any exist and the audit runs first:** read them as current governance inputs and do not write anything yet
...
If any of these are missing from the PRD and relevant to this project, mark them with `[NEEDS PRD CLARIFICATION]` for the interview or audit summary. Items the Adaptation Guidelines let you skip (a content model for a CLI, design tokens without a UI) are left out, not marked. Record open PRD questions in `MEMORY.md` *Current State* as blockers or open questions.
...
3. **Task management**: GitHub Issues, Linear, Jira, or just `MEMORY.md`?
   ...
   - If GitHub Issues are chosen but the repository has no GitHub remote, mark `[USER DECISION REQUIRED]` (connect a remote, or use a local task list)
   - If the pipeline will select issues by label, ask for that eligibility label (the *Auto-Develop Policy* `{auto-label}`)
...
5. **Package manager / build tool**: npm, pnpm, yarn, bun, uv, poetry, cargo, go, make?
   - Default suggestion for Node.js: pnpm; for other stacks there is no default, ask

6. **Automation**: Does the project use or plan CI/CD, auto-develop scripts, automated issue processing, or guardrail hooks?
   - If it uses (or plans to use) the pipeline generated by the automate mode, also ask two optional follow-ups; both are fail-safe and can be skipped (formats in `references/contract.md` sections 4 and 5):
...
- If the project has automation (CI/CD, auto-develop scripts), add an "Auto-Develop Policy" section. See `references/agents-template.md` for a concrete example. It must state every memory rule M1 to M7 (`references/contract.md` section 3) and the rollback exception that keeps *Prohibited Actions* consistent with the pipeline's own failure-path discards; the automate mode stops with `[NEEDS GOVERNANCE]` when M1 to M5 are missing
- Encode phase order in the *Phase Plan* as "depends on Phase N"; the automate mode turns it into `Depends on #N` once issue numbers exist
...
- If the project uses the automate mode's test gate, add a line `TARGETED_TEST_CMD='<command>'` inside the Development Commands block with a literal, unquoted `{TARGET}` token (e.g. `TARGETED_TEST_CMD='pytest {TARGET}'`). Include it only when AGENTS.md sets `TEST_POLICY` to something other than `off`; `TEST_POLICY=required` without it makes the pipeline degrade to `preferred` and emit `[GOVERNANCE DRIFT]`. If its tool is not installed yet, mark it `# planned` like the other planned commands (`references/contract.md` section 5)
...
9. **Update Rules** - how to maintain this file (the memory rules in `references/contract.md` section 3)
...
### Step 9: Audit Before Updating

When governance files already exist, do not jump straight to rewriting them. Run the governance half of the audit as described in `references/audit.md` (read the four files, the PRD, and the repository; classify mismatches as Missing, Outdated, Contradicted by codebase, or Needs user decision; tag significant conflicts `[GOVERNANCE DRIFT]`; check the automation-contract coherence from `references/contract.md` section 6). Present the audit findings before proposing edits. Only after user approval, update the selected governance files with the merge strategy in Step 10.
...
4. In Update/merge, summarize the drift categories and the most important mismatches
...
Only after explicit approval, write the selected files: `SOUL.md`, `AGENTS.md`, and `CLAUDE.md` first, `MEMORY.md` and `memory/completed-phases.md` last. Add the "Governance files drafted" (or governance audit/update) entry to the archive as part of that last write, and only if the earlier files were written successfully. Do not commit; let the user review first.

If the user now wants the build loop automated, switch to the automate mode (`references/automate.md`); it requires all four files to exist.
...
- **No implementation yet?** -> Generate should still work; just mark planned commands and unresolved areas honestly

The templates in `references/` are structural blueprints, not rigid forms. Adapt them to fit the project.

## Quality Checklist

Before presenting files or an update proposal, verify:

- [ ] Mode and sub-mode were stated clearly (`govern`: `Generate` vs `Update/merge`)
...
- [ ] If AGENTS.md has an *Auto-Develop Policy*, it states M1 to M7 and the rollback exception (`references/contract.md` section 3)
- [ ] Any automation-contract fields present (AGENTS.md *Skill Policy*, `TEST_POLICY`/`TEST_ELIGIBILITY`, CLAUDE.md `TARGETED_TEST_CMD`) are coherent across files and use the exact `<type>:<pattern>=<value>` matcher form; absent fields were left absent rather than invented
