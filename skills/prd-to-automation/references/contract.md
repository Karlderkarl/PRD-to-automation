...
- `MEMORY.md` above roughly 15,000 characters is a drift finding; the remedy is an archive split, never deletion. The threshold is this skill's own budget, not a platform limit: `CLAUDE.md` imports `MEMORY.md` into every session and the pipeline's agents read it on every step, so every character is paid for on every turn.
...
| *Roles* and models | AGENTS.md, CLAUDE.md | suggested default only; a generic name (a provider or CLI such as "Claude" or "Codex") covers every model of that provider, a concrete name ("opus") only that model | user is asked in automate Step 3 regardless | user's pick differs (another provider or CLI than a generic name, or another model than a concrete name; "opus" for "Claude" does not differ): generated as chosen and recorded as open `[GOVERNANCE DRIFT]`; the user decides whether govern records the choice or automate Sync reverts to the governance |
